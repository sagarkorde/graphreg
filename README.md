...
5. **Elliptic benchmark evaluation** (`elliptic_benchmark.py`) — the same model family, retrained from
   ...
   under a strict temporal train/test split, with no parameter transfer from the mainnet stage.
...
Place a mainnet transaction Parquet file at `datasets/custom/Dataset.parquet` and the Elliptic CSVs under
`datasets/elliptic/`, then run:
